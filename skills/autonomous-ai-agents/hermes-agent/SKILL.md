...
# Suppress banner/spinner (quiet mode)
hermes -Q          # or hermes chat -Q "your question"
```
...
- **Authentication issues**: See `references/gateway-auth-troubleshooting.md` for detailed token/auth problems and fixes

#### Platform authentication issues
If you see "Unauthorized" or "Improper token has been passed" errors in gateway logs:
1. **Telegram**: "Unauthorized" when sending messages → Check `TELEGRAM_BOT_TOKEN` in `~/.hermes/.env` or run `hermes gateway setup` → Telegram to reconfigure
2. **Discord**: "Improper token has been passed" → Check `DISCORD_BOT_TOKEN` in `~/.hermes/.env` or run `hermes gateway setup` → Discord to reconfigure
3. **Other platforms**: Similar token/auth issues → Verify credentials in `~/.hermes/.env` or reconfigure via `hermes gateway setup`
4. After updating credentials, restart gateway: `hermes gateway restart`
